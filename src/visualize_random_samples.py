...
from table_datasets import PDFTablesDataset, read_pascal_voc
...
def plot_img_with_bboxes(image, bboxes, classes, output_path, class_labels, xyxy = True):
    ...
    plt.savefig(output_path)  ## show the plot
...
def main(cfg):
    ...
    for samples, targets in data_loader_test:
        ...
        plot_results(samples, targets, outputs, results, dataset_test, args.output_path)

@torch.no_grad()
def plot_results(samples, targets, outputs, results, dataset, output_path, threshold = 0.5):
    for mask, sample, target, result, output in zip(samples.mask, samples.tensors, targets, results,
                                                    outputs['pred_boxes']):
        s = np.array(sample)
        m = np.array(mask)
        s = s[:, ~m.T[0], :]
        ...
        plot_img_with_bboxes(
            ...
            class_labels=['table', 'table column', 'table row', 'table column header',
                          'table projected row header', 'table spanning cell', 'no object'],
            output_path=os.path.join(output_path, f'{img_name}_pred'),
            xyxy=False,
        )
